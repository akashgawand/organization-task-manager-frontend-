...
export default function TeamLeadDashboard() {
  ...
  const teamProjects = mockProjects.filter(
    (p) => p.ownerId === user.id || p.teamId === "1",
  );
  const allProjectTasks = teamProjects.flatMap((p) => getTasksByProject(p.id));

  const [selectedProject, setSelectedProject] = useState(teamProjects[0]?.id);
  ...
  const inProgressTasks = allProjectTasks.filter(
    (t) => t.status === "in_progress",
  ).length;
  const reviewTasks = allProjectTasks.filter(
    (t) => t.status === "review",
  ).length;
                  ...
                  <div key={member.userId}>
                    ...
                    <div className="h-1.5 bg-[rgb(var(--color-border))] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(member.activeTasks / 5) * 100}%` }}
                      />
                    </div>
                  </div>
                );
