...
export default function UserFilterSelect({ value, onChange, users }: UserFilterSelectProps) {
    ...
    const selected = isAll ? null : users.find((u) => String(u.id) === String(value));
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                ...
                className={`w-64 flex items-center justify-between px-4 py-2.5 bg-[rgb(var(--color-surface))] border rounded-xl text-sm transition-all duration-200 cursor-pointer ${isOpen
                    ? "border-[rgb(var(--color-accent))] ring-2 ring-[rgb(var(--color-accent))]/30 shadow-md"
                    : "border-[rgb(var(--color-border))] hover:border-[rgb(var(--color-border-hover))] shadow-sm"
                    }`}
            >
                {isAll ? (
                    <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[rgb(var(--color-text-primary))] font-medium">All Team Members</span>
                    </div>
                ) : selected ? (
                    <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-[rgb(var(--color-accent))]/10 text-[rgb(var(--color-accent))] flex items-center justify-center text-xs font-bold">
                        ...
                        <span className="text-[rgb(var(--color-text-primary))] font-medium truncate max-w-[120px]">{selected.name}</span>
                    </div>
                ) : (
                    <span className="text-[rgb(var(--color-text-tertiary))]">Select user</span>
                )}
            ...
            {/* Dropdown */}
            {isOpen && (
                <div className="absolute right-0 top-[calc(100%+6px)] w-72 bg-[rgb(var(--color-surface))] border border-[rgb(var(--color-border))] rounded-xl shadow-xl shadow-black/10 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                    {/* Search */}
                    ...
                    <div className="max-h-64 overflow-y-auto">
                        ...
                        {!search && (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-all duration-150 cursor-pointer border-b border-[rgb(var(--color-border))]/50 ${isAll ? "bg-[rgba(var(--color-accent),0.08)]" : "hover:bg-[rgb(var(--color-surface-hover))]"
                                    }`}
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className={`text-sm font-medium truncate ${isAll ? "text-[rgb(var(--color-accent))]" : "text-[rgb(var(--color-text-primary))]"}`}>
                                        All Team Members
                                    </p>
                                    <p className="text-[10px] text-[rgb(var(--color-text-tertiary))] truncate">Company overview</p>
                                </div>
                                {isAll && <Check className="w-4 h-4 text-[rgb(var(--color-accent))] shrink-0" />}
                            </button>
                        )}
